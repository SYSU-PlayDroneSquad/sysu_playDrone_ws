...
class ST8(object):
    def __init__(self):
        ...
        self.tar_x, self.tar_y = [2.5, 2.5], [5, 20]  # --------------------------[撤离点坐标_需实际修改]
    ...
    def back(self, k, bm):
        if self.juz[bm] > self.der:
            # if self.pz[k] < self.z + (self.zt - self.z) / 2:
            #     if self.pvz[k] < (self.v_bre/3):
            #         az = self.a_bri
            #     else:
            #         az = 0
            # elif self.pz[k] >= self.z + (self.zt - self.z) / 2 and (self.pz[k] < self.zt):
            #     if self.pz[k] >= (self.zt - (self.v_bre/3)**2 / (2 * self.a_bri)):
            #         az = -self.a_bri
            #     else:
            #         az = 0
            ...
            if self.ct[bm] % 4 == 0:
                self.oxx[bm] = self.ox[bm] / 4
                self.oyy[bm] = self.oy[bm] / 4
                # print('oxx:', self.oxx)
                # print('')
                # print('oyy:', self.oyy)
                # print('')
    ...
    def srk(self, k, dm):
        self.dis[k] = np.sqrt((self.oex - self.px[k]) ** 2 + (self.oey - self.py[k]) ** 2)
        self.err[k] = self.r0 - self.dis[k]
        self.ver_xi[k] = (self.oex - self.px[k]) / np.sqrt((self.oex - self.px[k]) ** 2 + (self.oey - self.py[k]) ** 2)
        self.ver_yi[k] = (self.oey - self.py[k]) / np.sqrt((self.oex - self.px[k]) ** 2 + (self.oey - self.py[k]) ** 2)
        self.ter[dm] += self.err[k]
        ...
        if self.dis[k] > (self.r0 + (self.r1 - self.r0) / 2):  # self.dis[k] <= self.r1 + 5 * self.der and
            ...
            if self.dis[k] <= (self.r0 + self.v_bre**2 / (2 * self.a)):
                if jud_x > 0:
                    aa = -0.1 * self.a  # 带一点点减速度
                else:
                    aa = 0.1 * self.a  # 用于修正位置
            else:
                aa = 0
        else:
            # 进入优化程序
