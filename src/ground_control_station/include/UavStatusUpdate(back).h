...
#include <sstream>
// customer include
#include "ground_control_station/VisualStatus.h"
#include "ground_control_station/VisualStatusArray.h"
...
public:
    ...
    void sort(int empty) const{
        ...
        for(int i = exist - 1; i >= 1; i--){
            int b = std::atoi(_status[i][0].substr(2, 2).c_str());
            int a = std::atoi(_status[i - 1][0].substr(2, 2).c_str());
            if(b < a){
                tmp = _status[i][0];
